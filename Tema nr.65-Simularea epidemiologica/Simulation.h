...
private:
	...
	int last_infected_count; // Last count of infected people
...
public:
	...
	void runSimulation(); // Simulation loop

	void getStateCounts(int& healthy, int& infected, int& immune, int& quarantined);  // Count the number of people in each state

	void displaySimulationStatus(); // Print the number of people in each state

	void adjustInfectionRate(); // Adjust the infection rate based on the number of infected people

	void initializeInfected(); // Set the initial infected people

	bool hasActiveInfections(); // Check if there are any infected people
